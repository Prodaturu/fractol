...
/*   Updated: 2024/06/02 15:24:48 by sprodatu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * 		Zoom in ante xmin xmax ymin ymax center ki daggaraki move
 * avutayi ante manam xmax ni tagiyali
 */
 ...
 * 		sets the xmin, xmax, ymin, ymax, cre, cim values for the Julia
 * fractal. If arguments are un-valid, it prints an error message and exits the
 * program.
 *
 ...
 * 		or EXIT_FAILURE (1) if there is a error or no command-line
 *arguments.
 *
 ...
 * 		-> parameters:- MLX library instance, zoom (cust-fn), t_fractal
 *structure
 *
 ...
 * 		-> parameters:- MLX library instance, key function, t_fractal
 *structure
 *
 ...
 * 			-> parameters:- MLX library instance, t_fractal
 * structure
 * 				-> initializes the t_fractal structure
 * 				-> creates a new image
 * 				-> checks if the image was created successfully
 * 				-> checks if the MLX library instance is
 * initialized successfully Free the t_fractal structure else Print an error
 * message Return EXIT_FAILURE i.e 1
 */
 ...
 * resize(bool)
 * 				-> It takes the width, height, and title of the
 * window
 * 				-> It returns a pointer to the MLX library
 * instance
 * 				-> The parameters are pre-defined in MLX library
 * header file
 * 					-> WIDTH, HEIGHT, TITLE
 * 				-> The resize parameter is set to true
 * 					-> true: allows the window to be resized
 * 					-> false: does not allow the window to
 * be resized
 * 					-> As we want the window to be
 * resizable, we set it to true
 * 				-> mlx_init returns a pointer to the MLX library
 * instance Allocate memory for a t_fractal structure
 * 			-> malloc, standard lib-fn is used to allocate memory
 * 			-> this t_madel structure is used to store fractal data
 * 			-> sizeof(t_fractal) is the size of the t_fractal
 * structure
 * 			-> malloc returns a pointer to the allocated memory
 * 			-> This is defined in the fractol.h header file
 * 		cleanup & initialize  t_fractal structure
 * 			-> img_modhalu cust-fn initializes & cleansup t_fractal
 * struct
 * 			-> parameters:- MLX library instance, t_fractal
 * structure
 * 				-> initializes the t_fractal structure
 ...
 * 					-> It returns 1 if the initialization is
 * successful
 * 					-> It returns EXIT_FAILURE if there is
 * an error
 * 						-> EXIT_FAILURE: indicates there
 * is an error
 * 			-> If the initialization is successful
 * 				-> Register the zoom hook
 * 					-> mlx_scroll_hook, MLX lib-fn is used
 * to register the zoom hook
 * 					-> parameters:- MLX library instance,
 * zoom (cust-fn), t_fractal structure
 * 						-> registers the zoom hook
 * 						-> calls the zoom function when
 * the mouse is scrolled
 * 				-> Initialize the t_fractal struct
 * 					-> ft_init, cust-fn is used to
 * initialize the t_fractal structure
 * 					-> parameters:- t_fractal struct, argv,
 * MLX lib instance, argc
 * 						-> initializes t_fractal struct
 * based on command-line arguments
 * 						-> sets xmin xmax ymin ymax
 * values for Mandelbrot fractal
 * 						-> sets xmin xmax ymin ymax cre
 * & cim values for Julia fractal
 * 						-> if arguments are un-valid
 * print error_msg & exit program
 * 				-> Register the key hook
 * 					-> mlx_key_hook, MLX lib-fn is used to
 * register the key hook
 * 					-> parameters:- MLX library instance,
 * key function, t_fractal structure
 * 						-> It registers the key hook
 * 						-> It calls the key function
 * when a key is pressed
 * 				-> Enter the main event loop
 * 					-> mlx_loop, MLX lib-fn is used to enter
 * the main event loop
 * 					-> parameters:- MLX library instance
 * 						-> It enters the main event loop
 * 						-> It calls the registered hooks
 * and event handlers
 * 				-> Terminate the MLX library
 * 					-> mlx_terminate, MLX lib-fn is used to
 * terminate the MLX library
 * 					-> parameters:- MLX library instance
 * 						-> It terminates the MLX library
 * 		Free the t_fractal structure
 * 			-> free, standard lib-fn is used to free the allocated
 * memory
 * 			-> It frees the memory allocated for the t_fractal
 * structure else Print an error message Return EXIT_FAILURE i.e 1
 */
