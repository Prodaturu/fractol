...
typedef enum mlx_errno {
  ...
  MLX_INVIMG, // The provided image is invalid, might indicate mismanagement of
              // images.
...
// Set these values, if necessary, before calling `mlx_init` as they define the
// behaviour of MLX42.
typedef enum mlx_settings {
  MLX_STRETCH_IMAGE = 0, // Should images resize with the window as it's being
                         // resized or not. Default: false
  MLX_FULLSCREEN, // Should the window be in Fullscreen, note it will fullscreen
                  // at the given resolution. Default: false
  MLX_MAXIMIZED,  // Start the window in a maximized state, overwrites the
                  // fullscreen state if this is true. Default: false
  MLX_DECORATED,  // Have the window be decorated with a window bar. Default:
                  // true
  MLX_HEADLESS,   // Run in headless mode, no window is created. (NOTE: Still
 ...
 * implementation. You can find the ACTUAL cursor in the following files at GLFW
 * named under *_platform.h
 */
 ...
 * @param[in] value Settings value to determine the state of the setting. Can be
 * a boolean or an enum / macro.
 */
 ...
 *
 * NOTE: In MacOS this function does nothing, you should use the bundles icon to
 * set the dock bar icon.
 ...
 *
 * Pass -1 for no limit to any of the min/max parameters to ignore that
 * boundary. For instance if you want a min window size but the max window size
 * can be whatever.
 *
 * @param[in] mlx The MLX instance handle.
 ...
 *
 * NOTE: Keep in mind that images that are on the same Z layer cut each other
 * off. so if you don't see your image anymore make sure it's not conflicting by
 * being on the same layer as another image.
 *
