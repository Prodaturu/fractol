...
// NOTE: This test cannot be run with a fixture because the settings need to be
...
TEST_F(Window, SingleImage) {
  ...
  int32_t val =
      mlx_image_to_window(mlx, img, Window::width / 4, Window::height / 4);
  EXPECT_GE(val, 0);
  ASSERT_EQ(mlx_errno, MLX_SUCCESS);
...
TEST_F(Window, MultipleImages) {
  ...
  int32_t val1 =
      mlx_image_to_window(mlx, img1, Window::width / 4, Window::height / 4);
  EXPECT_GE(val1, 0);
  ASSERT_EQ(mlx_errno, MLX_SUCCESS);
