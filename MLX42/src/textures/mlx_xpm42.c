...
static bool mlx_read_data(xpm_t *xpm, FILE *file, uint32_t *ctable, size_t s) {
  ...
  for (int64_t y_xpm = 0; y_xpm < xpm->texture.height; y_xpm++) {
    ...
    for (int64_t x_xpm = 0, x_line = 0; x_xpm < xpm->texture.width;
         x_xpm++, x_line += xpm->cpp) {
      uint8_t *pixelstart =
          &xpm->texture.pixels[(y_xpm * xpm->texture.width + x_xpm) * BPP];
      mlx_draw_pixel(pixelstart,
                     ctable[mlx_fnv_hash(&line[x_line], xpm->cpp) % s]);
    }
  }
  free(line);
  return (true);
...
static bool mlx_read_xpm_header(xpm_t *xpm, FILE *file) {
  ...
  xpm->texture.pixels =
      calloc(xpm->texture.width * xpm->texture.height, sizeof(int32_t));
