 ...
 * Since the same output pointer is reused it should only be freed once, either
 * on success or failure. This function is made to be somewhat similar to
 * getline. Getline can't be used directly since it's not standard and therefore
...
double mlx_get_time(void) { return (glfwGetTime()); }
