...
void mlx_flush_batch(mlx_ctx_t *mlx) {
  ...
  glBufferData(GL_ARRAY_BUFFER, mlx->batch_size * sizeof(vertex_t),
               mlx->batch_vertices, GL_STATIC_DRAW);
...
static int8_t mlx_bind_texture(mlx_ctx_t *mlx, mlx_image_t *img) {
  ...
  // If no free slot was found, flush the batch and assign the texture to the
...
mlx_instance_t *mlx_grow_instances(mlx_image_t *img, bool *did_realloc) {
  ...
  if (img->count >= ctx->instances_capacity) {
    ...
    *did_realloc = true;
    return realloc(img->instances,
                   ctx->instances_capacity * sizeof(mlx_instance_t));
  }
  *did_realloc = false;
...
void mlx_delete_image(mlx_t *mlx, mlx_image_t *image) {
  ...
  if ((imglst = mlx_lstremove(&mlxctx->images, image, &mlx_equal_image))) {
    ...
    mlx_freen(5, image->pixels, image->instances, image->context, imglst,
              image);
  }
}
  ...
  if (nwidth != img->width || nheight != img->height) {
    ...
    for (uint32_t j = 0; j < nheight; j++)
      for (uint32_t i = 0; i < nwidth; i++)
        destin[j * nwidth + i] =
            origin[(uint32_t)(j * hstep) * img->width + (uint32_t)(i * wstep)];
