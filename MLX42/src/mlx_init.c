...
static bool mlx_create_buffers(mlx_t *mlx) {
  ...
  // Texture index
  glVertexAttribIPointer(2, 1, GL_BYTE, sizeof(vertex_t),
                         (void *)(sizeof(float) * 5));
...
mlx_t *mlx_init(int32_t width, int32_t height, const char *title, bool resize) {
  ...
  if (!(mlx->window = glfwCreateWindow(
            width, height, title,
            mlx_settings[MLX_FULLSCREEN] ? glfwGetPrimaryMonitor() : NULL,
            NULL)))
...
void mlx_set_setting(mlx_settings_t setting, int32_t value) {
  MLX_ASSERT(setting >= 0 && setting < MLX_SETTINGS_MAX,
